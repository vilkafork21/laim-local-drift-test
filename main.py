...
def html_report_valtest_local_drift(res: dict, semaphore_title: str) -> str:
    ...
    return render_test_report(
        "6.3.6", "Тест на локальный дрифт запросов",
        ...
        ("СЗ выше E.", f"Не менее {MIN_OOS_SAMPLES} объектов в эталонной корзине OOS.",
         "Метки качества бинарные или непрерывные в известной шкале [0; 1]."),
        [("green", f"Абсолютное снижение D < {format_report_number(pre.get('semaphore_threshold', (0.5, 0.8))[0])} "
          "и зелёный светофор КМ на эталоне"),
         ("yellow", "Иначе"),
         ("red", f"Абсолютное снижение D ≥ {format_report_number(pre.get('semaphore_threshold', (0.5, 0.8))[1])} "
          "или красный светофор КМ на эталоне"),
         ("gray", "Недостаточная близость запросов, высокая доля непокрытых запросов, "
          "нехватка данных или неизвестная шкала метрики")],
        algorithm=(
            "Для каждого запроса мониторинга подобрать N ближайших запросов эталонной корзины.",
            "Оценить ожидаемое качество по близости запросов и меткам эталона, затем усреднить оценки.",
            "Рассчитать статистики близости и долю запросов, не покрытых эталонной корзиной.",
        ),
